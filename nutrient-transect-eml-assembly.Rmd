...
nut_url <- glue::glue('{base_api_url}/nut/all.csv')
print(nut_url)

nut_all <- tryCatch(read_csv(nut_url), 
                 error=function(cond) 
                     { message(cond)
                       return(NULL) })
...
```{r}
nut_all <- nut_all[order(nut_all$date), ]

# rename "distance_km" 
names(nut_all)[names(nut_all) == 'distance_km'] <- 'station_distance'
names(nut_all)[names(nut_all) == 'date'] <- 'date_time_utc'
...
read_ooi_discrete <- function(log_url) {
  ...
  readr::read_csv(logtf) |>
    clean_names() |>
    select(
      cruise, target_asset, start_latitude_degrees, start_longitude_degrees, cast, 
      niskin_bottle_position, ctd_bottle_closure_time_utc, ctd_depth_m,
      ) |>
    rename(
      date_time_utc = ctd_bottle_closure_time_utc,
      niskin        = niskin_bottle_position,
      latitude      = start_latitude_degrees,
      longitude     = start_longitude_degrees,
      depth         = ctd_depth_m
      ) |>
...
AR52ooi <- read_ooi_discrete(log_url_ar52)
AR52ooi <- distinct(AR52ooi)

AR52ooi <- AR52ooi |>
  mutate(target_asset = case_when(startsWith(target_asset, "LTER") ~ target_asset, TRUE ~ "")) |>
  rename(nearest_station = target_asset)

AR52ooi$nearest_station <- str_remove(AR52ooi$nearest_station, "LTER ")
AR52ooi$nearest_station <- str_remove(AR52ooi$nearest_station, "LTER")

# get nutrient values from API
#ar52_url <- glue::glue('{base_api_url}nut/ar52_nutrient_samplelog.csv')
#AR52lter <- read_csv(ar52_url)
AR52lter <- read_csv('ar52_nutrient_samplelog.csv')
AR52lter <- distinct(AR52lter)
AR52lter$flag_nitrate_nitrite <- as.integer(2)
AR52lter$flag_ammonium <- as.integer(2)
AR52lter$flag_phosphate <- as.integer(2)
AR52lter$flag_silicate <- as.integer(2)
AR52lter$project_id <- as.character("OOI")

AR52 <- left_join(AR52lter, AR52ooi, by = c("cruise", "cast", "niskin"))

all_cruises <- full_join(nut_all, AR52)
...
need_metadata <- need_metadata |> rename(date_time_utc = date) 
...
manual_flags <- read_csv('LTERnut_manual_flag_correction_20260331.csv')
...
AR52_check <- all_cruises |> filter(cruise %in% c("AR52A", "AR52B"))
...
```
## Remove row with most fields missing
```{r}

all_nutrients <- all_nutrients |> filter(!is.na(date_time_utc))
# there is a row for AR66A cast 4 Niskin 1 rep b with no other data or metadata
