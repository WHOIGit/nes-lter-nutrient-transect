...
# compare between old and new cruises read in from updated API
...
nutrient_file <- "nes-lter-nutrient-transect"
info <- "nutrient-transect-info.xlsx"
# info_sheets <- c("ColumnHeaders", "Personnel", "Keywords", "CustomUnits")

# read in nutrient info
headers <- read_excel(path = paste0(dir, "/", info), 
                  sheet = "ColumnHeaders", na = "NA")
personnel <- read_excel(path = paste0(dir, "/", info), 
                  sheet = "Personnel", na = "NA")
keywords <- read_excel(path = paste0(dir, "/", info), 
                  sheet = "Keywords", na = "NA")
custom_units <- read_excel(path = paste0(dir, "/", info), 
...
write.table(headers, paste0("attributes_", nutrient_file, ".txt"), 
            quote = FALSE, na = "", sep = "\t", row.names = FALSE)
...
# Import EDI templates for diet dataset licensed under CC0
import_templates(path = dir,
                 license = "CC0",
                 data.files = paste0(nutrient_file, ".csv"))
...
make_eml(path = dir,
         ...
         data.table = paste0(nutrient_file, ".csv"),
