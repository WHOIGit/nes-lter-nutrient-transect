...
Check that the updated API hasn't produced data that diverges from a past timepoint. If there is no output, the data has passed the comparison test. UPDATE: Test passed, code commented out.

```{r}
# # define columns to compare
# num_cols <- c("nitrate_nitrite", "ammonium", "phosphate", "silicate", "depth")
# 
# # subset new cruises read in from updated API
# all_cruises_new <- all_cruises %>% 
#   filter(cruise != "EN644" & cruise != "AR22" & cruise !=  "AR32") %>%
#   select(num_cols)
# # read in old cruises
...
#   val_check <- round(all_cruises_new[,i], 3) == round(all_cruises_old[,i], 3)
#   
#   # report if any fail the check
#   if (isTRUE("FALSE" %in% val_check)) {
#     print(paste0(names(all_cruises_new[,i]), " contains a mismatch at row(s): ", 
...
st_dist_outliers <- all_cruises %>% filter(station_distance > 2)
