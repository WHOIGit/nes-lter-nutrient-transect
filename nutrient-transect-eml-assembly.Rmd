...
libs <- c("tidyverse", "readxl", "lubridate", "devtools", "EMLassemblyline", "EML", "maps", "xml2", "reticulate")
# load libraries
lapply(libs, require, character.only = TRUE)

```

## Compile NES-LTER Cruise Data (REDACTED)

REDACTED because the cruise data is compiled using a Python script that additionally determines the project ID, nearest station, and station distance.

This chunk concatenates a data table of the specified cruises called from the Rest API. 
...
#   # create list of cruises to acquire data from
#   cruiselist <- c("en608","en617", "en627", "en644", "ar22", "ar32")
#   # load in the initial cruise to which the other cruises will be added
#   first_cruise <- read_csv(paste0("https://nes-lter-data.whoi.edu/api/nut/", cruiselist[1], ".csv"))
#   # load associated ctd metadata
#   first_ctd <- read_csv(paste0("https://nes-lter-data.whoi.edu/api/ctd/", cruiselist[1], "/metadata.csv")) %>%
#     select(-date, -latitude, -longitude)
#   # merge data frames
#   prev_all <- left_join(first_cruise, first_ctd, by = c("cruise", "cast"))
# 
#   # loop through cruiselist to compile cruise data into one file
#   for (k in 2:length(cruiselist)){
#     # read in cruise data
#     next_cruise <- read_csv(paste0("https://nes-lter-data.whoi.edu/api/nut/", cruiselist[k], ".csv"))
#     # read in ctd metadata
#     next_ctd <- read_csv(paste0("https://nes-lter-data.whoi.edu/api/ctd/", cruiselist[k], "/metadata.csv")) %>%
#       select(-date, -latitude, -longitude)
#     # merge cruies and ctd metadata
#     next_all <- left_join(next_cruise, next_ctd, by = c("cruise", "cast"))
# 
#     # bind the next cruise to the compiled cruise dataset
#     all_cruises <- rbind(prev_all, next_all)
# 
#     # if statment to reset the previous cruises until all cruises are read in
#       if(k < length(cruiselist)) {
#         prev_all <- all_cruises
#       }
#   }
...
#   all_cruises$distance_km <- round(all_cruises$distance_km, 2)
# 
#   # set missing values in distance to -9999
#   all_cruises$distance_km[is.na(all_cruises$distance_km)] <- -9999
# 
...
#   # assumes that nutrient data is already rounded
# }

```

## Evaluate Python Script to Generate Compiled Cruise Data
IN PROGRESS. Alternative sample ID, project ID, nearest station, and station distance were determined for each sample using python script from a Jupyter Notebook developed by Joe Futrelle. This output was saved as a csv in the working directory and will now be input. 
...
This chunk loads in the cruise data compiled by nearest_station_generalized.ipynb
```{r}
